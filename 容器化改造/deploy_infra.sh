...
read -p "请输入TKE附加组件版本（默认1.0.7）: " TKE_ADDON_VERSION
TKE_ADDON_VERSION=${TKE_ADDON_VERSION:-"1.0.7"}
...
# 验证高可用状态
HA_STATUS=$(terraform output -json ha_status)
PRIMARY_NODES=$(echo $HA_STATUS | jq -r '.node_distribution.primary_az')
SECONDARY_NODES=$(echo $HA_STATUS | jq -r '.node_distribution.secondary_az')

echo "=== 高可用状态 ==="
echo "主可用区节点数: $PRIMARY_NODES"
echo "备可用区节点数: $SECONDARY_NODES"